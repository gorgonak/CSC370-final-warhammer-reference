...
class Main(wx.Frame):
    def __init__(self, parent, title: str, gui: 'ChatbotGUI', gif_path: str, show_timestamp: bool = False):
        ...
        # input chat label
        self.input_chat_label = wx.StaticText(self.io_panel, label="Talk to Reginald:")

        # input chat text box
        self.input_box = wx.TextCtrl(self.io_panel, style=wx.TE_PROCESS_ENTER, size=wx.Size(400, 20))

        # input chat enter button
        self.chat_button = wx.Button(self.io_panel, label="Send your Text to Reginald")

        # input chat label
        self.output_chat_label = wx.StaticText(self.io_panel, label="Reginald Response History:")

        # AI chat History
        self.output_chat = wx.TextCtrl(self.io_panel, size=wx.Size(400, 145), style=wx.TE_READONLY | wx.TE_MULTILINE)

        # ai status label
        self.ai_status_label = wx.StaticText(self.io_panel, label="Reginald's Current Status:")
    ...
    def update_message_history(self):
        ...
        # for message in ai_message_history:
           # user_text += message + "\n"

        # aggregate ai messages
        for message in self.ai_message_history[::-1]:
            ai_text += str(message) + "\n"
    ...
    def on_send_press(self, event):
        ...
        # add the message to message history
        self.ai_message_history.insert(0, self.get_timestamp() + "You> " + text)
...
class ChatbotGUI:
    def __init__(self, title: str, gif_path: str, show_timestamps: bool = True, default_voice_options: dict = None):
        ...
        if default_voice_options is None:
            ...
            self.default_voice_options = {
                "rate": default_voice_options.get("rate", 135),
                "volume": default_voice_options.get("rate", 1),
